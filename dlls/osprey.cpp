...
void COsprey :: FindAllThink( void )
{
	...
	m_iUnits = 0;
	while (m_iUnits < MAX_CARRY && (pEntity = UTIL_FindEntityByClassname( pEntity, "monster_human_grunt" )) != 0)
	{
		if (pEntity->IsAlive())
		{
...
void COsprey::FlyThink( void )
{
	...
	if ( m_pGoalEnt == 0 && !FStringNull(pev->target) )// this monster has a target
	{
	...
	if (gpGlobals->time > m_startTime + m_dTime)
	{
		...
		do {
			m_pGoalEnt = CBaseEntity::Instance( FIND_ENTITY_BY_TARGETNAME ( 0, STRING( m_pGoalEnt->pev->target ) ) );
...
void COsprey::Flight( )
{
	...
	else
	{
		...
		pPlayer = UTIL_FindEntityByClassname( 0, "player" );
