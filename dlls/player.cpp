...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	SetSuitUpdate(0, FALSE, 0);

	// send "health" update message to zero
	m_iClientHealth = 0;
	MESSAGE_BEGIN( MSG_ONE, gmsgHealth, 0, pev );
...
void CBasePlayer::StartDeathCam( void )
{
	...
	pSpot = FIND_ENTITY_BY_CLASSNAME( 0, "info_intermission");	
...
void CBasePlayer::PlayerUse ( void )
{
	...
	CBaseEntity *pObject = 0;
	CBaseEntity *pClosest = 0;
	Vector		vecLOS;
	...
	while ((pObject = UTIL_FindEntityInSphere( pObject, pev->origin, PLAYER_SEARCH_RADIUS )) != 0)
	{
...
void CBasePlayer::UpdateStatusBar()
{
	...
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgStatusText, 0, pev );
			WRITE_BYTE( 0 );
			WRITE_STRING( sbuf0 );
		MESSAGE_END();
	...
	for (int i = 1; i < SBAR_END; i++)
	{
		...
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgStatusValue, 0, pev );
				WRITE_BYTE( i );
...
void CBasePlayer :: UpdateGeigerCounter( void )
{
	...
	if (range != m_igeigerRangePrev)
	{
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgGeigerRange, 0, pev );
			WRITE_BYTE( range );
		MESSAGE_END();
	}
...
void CBasePlayer::SetSuitUpdate(char *name, int fgroup, int iNoRepeatTime)
{
	...
	// if name == 0, then clear out the queue
	...
	if (!fgroup)
	{
		isentence = SENTENCEG_Lookup(name, 0);
		if (isentence < 0)
			return;
	}
	else
...
BOOL IsSpawnPointValid( CBaseEntity *pPlayer, CBaseEntity *pSpot )
{
	...
	while ( (ent = UTIL_FindEntityInSphere( ent, pSpot->pev->origin, 128 )) != 0 )
	{
...
inline int FNullEnt( CBaseEntity *ent ) { return (ent == 0) || FNullEnt( ent->edict() ); }
...
// choose a info_player_deathmatch point
	...
	{
		pSpot = UTIL_FindEntityByClassname(0, "info_player_start");
		if ( !FNullEnt(pSpot) )
			goto ReturnSpot;
	}
	else
	{
		pSpot = UTIL_FindEntityByTargetname( 0, STRING(gpGlobals->startspot) );
		if ( !FNullEnt(pSpot) )
			goto ReturnSpot;
	}
...
{
	if ( pev == 0 )		// Not fully connected yet
		return "";
...
void CBasePlayer::GiveNamedItem( const char *pszName )
{
	...
	if ( FNullEnt( pent ) )
	{
		ALERT ( at_console, "0 Ent in GiveNamedItem!\n" );
		return;
	}
...
void CBasePlayer :: FlashlightTurnOn( void )
{
	...
	if ( (pev->weapons & (1<<WEAPON_SUIT)) )
	{
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgFlashlight, 0, pev );
		WRITE_BYTE(1);
...
void CBasePlayer::ImpulseCommands( )
{
	...
	case 99:
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgLogo, 0, pev );
			WRITE_BYTE(iOn);
		MESSAGE_END();
	...
	case	201:// paint decal
		...
		{// line hit something, so paint a decal
			...
			CSprayCan *pCan = GetClassPtr((CSprayCan *)0);
			pCan->Spawn( pev );
		}
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case	202:// Random blood splatter
		...
		if ( tr.flFraction != 1.0 )
		{// line hit something, so paint a decal
			CBloodSplat *pBlood = GetClassPtr((CBloodSplat *)0);
			pBlood->Spawn( pev );
		}
		break;
...
int CBasePlayer :: GiveAmmo( int iCount, char *szName, int iMax )
{
	...
	if ( gmsgAmmoPickup )  // make sure the ammo messages have been linked first
	{
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgAmmoPickup, 0, pev );
...
void CBasePlayer::SendAmmoUpdate(void)
{
	for (int i=0; i < MAX_AMMO_SLOTS;i++)
	{
		if (m_rgAmmo[i] != m_rgAmmoLast[i])
		{
			...
			MESSAGE_BEGIN( MSG_ONE, gmsgAmmoX, 0, pev );
				WRITE_BYTE( i );
...
void CBasePlayer :: UpdateClientData( void )
{
	if (m_fInitHUD)
	{
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgResetHUD, 0, pev );
			WRITE_BYTE( 0 );
		MESSAGE_END();

		if ( !m_fGameHUDInitialized )
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgInitHUD, 0, pev );
			MESSAGE_END();
	...
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgHideWeapon, 0, pev );
	...
	if (gDisplayTitle)
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgShowGameTitle, 0, pev );
		WRITE_BYTE( 0 );
		MESSAGE_END();
		gDisplayTitle = 0;
	}
...
#define clamp( val, min, max ) ( ((val) > (max)) ? (max) : ( ((val) < (min)) ? (min) : (val) ) )
	...
	if (pev->armorvalue != m_iClientBattery)
	{
		...
		// send "health" update message
		MESSAGE_BEGIN( MSG_ONE, gmsgBattery, 0, pev );
	...
	if (pev->dmg_take || pev->dmg_save || m_bitsHUDDamage != m_bitsDamageType)
	{
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgDamage, 0, pev );
	...
	if ((m_flFlashLightTime) && (m_flFlashLightTime <= gpGlobals->time))
	{
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgFlashBattery, 0, pev );
		WRITE_BYTE(m_iFlashBattery);
		MESSAGE_END();
	}
	...
	if (m_iTrain & TRAIN_NEW)
	{
		ASSERT( gmsgTrain > 0 );
		// send "health" update message
		MESSAGE_BEGIN( MSG_ONE, gmsgTrain, 0, pev );
	...
	// ????		Icons
		...
		for (i = 0; i < MAX_WEAPONS; i++)
		{
			...
			MESSAGE_BEGIN( MSG_ONE, gmsgWeaponList, 0, pev );  
...
void CInfoIntermission::Think ( void )
{
	...
	// find my target
	pTarget = FIND_ENTITY_BY_TARGETNAME( 0, STRING(pev->target) );
