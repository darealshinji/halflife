...
void CBaseMonster :: Look ( int iDistance )
{
	...
	CBaseEntity	*pSightEnt = 0;// the current visible entity that we're dealing with
...
#if _DEBUG
		ALERT( at_error, "0 Return from PBestSound\n" );
#endif
		return 0;
	}
...
#if _DEBUG
	ALERT( at_error, "0 Return from PBestScent\n" );
#endif
	return 0;
}
...
BOOL CBaseMonster :: FRefreshRoute ( void )
{
	...
	switch( m_movementGoal )
	{
		...
		case MOVEGOAL_LOCATION:
			returnCode = BuildRoute( m_vecMoveGoal, bits_MF_TO_LOCATION, 0 );
			break;
...
//		DrawRoute( pev, m_Route, m_iRouteIndex, 0, 0, 255 );
	...
	for ( i = 0; i < count-1; i++ )
	{
		...
		else if ( CheckLocalMove ( vecStart, m_Route[m_iRouteIndex+i+1].vecLocation, pTargetEnt, 0 ) == LOCALMOVE_VALID )
		{
			// Skip vert
			continue;
		}
		else
		{
			...
			if ( CheckLocalMove ( vecStart, vecTest, pTargetEnt, 0 ) == LOCALMOVE_VALID )
			{
				outRoute[outCount].iType = iType;
				outRoute[outCount].vecLocation = vecTest;
			}
			else if ( CheckLocalMove ( vecSplit, vecTest, pTargetEnt, 0 ) == LOCALMOVE_VALID )
			{
				outRoute[outCount].iType = iType;
	...
	else
	{
		if ( ! (m_Route[ m_iRouteIndex ].iType & bits_MF_IS_GOAL) )
		{
			   ...
			   && (m_Route[m_iRouteIndex+1].iType & bits_MF_TO_NODE) == bits_MF_TO_NODE)
			{
				...
				if ( iLink >= 0 && WorldGraph.m_pLinkPool[iLink].m_pLinkEnt != 0 )
				{
...
// Debug, Draw the triangulation
		...
		if ( CheckLocalMove( pev->origin, vecRight, pTargetEnt, 0 ) == LOCALMOVE_VALID )
		{
			if ( CheckLocalMove ( vecRight, vecFarSide, pTargetEnt, 0 ) == LOCALMOVE_VALID )
			{
				if ( pApex )
				{
					*pApex = vecRight;
		...
		if ( CheckLocalMove( pev->origin, vecLeft, pTargetEnt, 0 ) == LOCALMOVE_VALID )
		{
			if ( CheckLocalMove ( vecLeft, vecFarSide, pTargetEnt, 0 ) == LOCALMOVE_VALID )
			{
				if ( pApex )
				{
					*pApex = vecLeft;
		...
		if (pev->movetype == MOVETYPE_FLY)
		{
			if ( CheckLocalMove( pev->origin, vecTop, pTargetEnt, 0 ) == LOCALMOVE_VALID)
			{
				if ( CheckLocalMove ( vecTop, vecFarSide, pTargetEnt, 0 ) == LOCALMOVE_VALID )
				{
					if ( pApex )
					{
						*pApex = vecTop;
...
#if 1
			if ( CheckLocalMove( pev->origin, vecBottom, pTargetEnt, 0 ) == LOCALMOVE_VALID )
			{
				if ( CheckLocalMove ( vecBottom, vecFarSide, pTargetEnt, 0 ) == LOCALMOVE_VALID )
				{
					if ( pApex )
					{
						*pApex = vecBottom;
...
void CBaseMonster :: StartMonster ( void )
{
	...
	if ( !FStringNull(pev->target) )// this monster has a target
	{
		...
		m_pGoalEnt = CBaseEntity::Instance( FIND_ENTITY_BY_TARGETNAME ( 0, STRING( pev->target ) ) );
		...
		if (WorldGraph.NextNodeInRoute( iMyNode, nodeNumber, iMyHullIndex, 0 ) != iMyNode)
		{
			...
			if ( flDist > flMinDist && flDist < flMaxDist)
			{
				...
				if (tr.flFraction == 1.0)
				{
					...
					if ( BuildRoute ( node.m_vecOrigin, bits_MF_TO_LOCATION, 0 ) )
					{
						flMaxDist = flDist;
...
BOOL CBaseMonster :: FindLateralCover ( const Vector &vecThreat, const Vector &vecViewOffset )
{
	...
	for ( i = 0 ; i < COVER_CHECKS ; i++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			if ( FValidateCover ( vecLeftTest ) && CheckLocalMove( pev->origin, vecLeftTest, 0, 0 ) == LOCALMOVE_VALID )
			{
		...
		if ( tr.flFraction != 1.0 )
		{
			if (  FValidateCover ( vecRightTest ) && CheckLocalMove( pev->origin, vecRightTest, 0, 0 ) == LOCALMOVE_VALID )
			{
...
BOOL CBaseMonster :: GetEnemy ( void )
{
	...
	if ( HasConditions(bits_COND_SEE_HATE | bits_COND_SEE_DISLIKE | bits_COND_SEE_NEMESIS) )
	{
		...
		if ( pNewEnemy != m_hEnemy && pNewEnemy != 0)
		{
